...
console.log(
  `Car 33 is a ${forCar33.car_year} ${forCar33.car_make} ${forCar33.car_model}`
);

// A better way that doesn't use for loops
const car33 = inventory.filter(car => car.id === 33)[0];
console.log(
...
console.log(
  `The last car is a ${inventory[lastCar].car_make} ${inventory[lastCar].car_model}`
);
...
//using map
const mapCarModelsSorted = inventory.map(car => car.car_model).sort();
console.log(mapCarModelsSorted);
...
const oldCars = [];
oldCars.push(...carYears.filter(year => year < 2000));
console.log(oldCars);
...
let BMWAndAudi = [];
BMWAndAudi.push(
  ...inventory.filter(car => car.car_make === "BMW" || car.car_make === "Audi")
);
